    ...
    for utt in doc:
        for cl in utt.lines:
            ...
            if form in form_to_entries:
                ...
                report_existing[doc.filename].append({
                    "before": original,
                    "lemma": lemma_id,
                    "category": "existing",
                ...
                report_existing[doc.filename].append({
                    "before": original, "lemma": lemma_id, "category": "new",
                    "multiple_candidates": False, "candidates": [],
                })
...
def process_unknown_words(
    ...
    for form, lines in unknown_forms.items():
        ...
        for cl in lines:
            ...
            original = cl.to_text()
            cl.insert_lemma(lemma_id)
            report_new[doc.filename].append({
                "before": original, "lemma": lemma_id, "category": "new",
                "multiple_candidates": False, "candidates": [],
            })
            stats["dict_matches_inserted"] += 1
            ...
            if not records_by_file:
                ...
                for fname, records in sorted(records_by_file.items()):
                    ...
                    for record in records:
                        fh.write(f"  [{record['lemma']}]\n")
                        fh.write(f"    {record['before']}\n")
        print(f"  [REPORT] {path}")
...
def process_files() -> None:
    ...
    # their numeric portions as well so newly generated IDs remain globally unique.
    import re
    for filename in os.listdir(TEXT_FOLDER):
        if filename.lower().endswith(".xml"):
            with open(os.path.join(TEXT_FOLDER, filename), encoding="utf-8") as fh:
                for number in re.findall(r'lemma="[A-Za-z]+(\d+)[a-z]*"', fh.read()):
                    id_gen.reserve(int(number))
    ...
    txt_files = sorted(f for f in os.listdir(TEXT_FOLDER) if f.lower().endswith(".xml"))
    if not txt_files:
        print(f"[INFO] No .xml files found in '{TEXT_FOLDER}'.")
        return
    ...
    # Save the dictionary without normalising unrelated existing entries.
    os.makedirs(OUTPUT_FOLDER, exist_ok=True)
    if OVERWRITE_SOURCE:
        dict_out = DICT_FILE
    else:
        dict_base = os.path.splitext(os.path.basename(DICT_FILE))[0]
        dict_out  = os.path.join(OUTPUT_FOLDER, f"{dict_base}_processed.xml")
    ...
    save_reports(OUTPUT_FOLDER, report_new, report_existing, report_new_entries)

    gui_records = []
    for filename, records in report_existing.items():
        xml_name = os.path.splitext(os.path.basename(filename))[0] + ".xml"
        gui_records.extend({"file": xml_name, **record} for record in records)
    for filename, records in report_new.items():
        xml_name = os.path.splitext(os.path.basename(filename))[0] + ".xml"
        gui_records.extend({"file": xml_name, **record} for record in records)
    with open(os.path.join(OUTPUT_FOLDER, "gui_results.json"), "w", encoding="utf-8") as fh:
        json.dump({"lines": gui_records}, fh, ensure_ascii=False, indent=2)
