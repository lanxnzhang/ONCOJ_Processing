...
### `src/coj` — Python package

A pure Python 3 package (no external dependencies) formalising the COJ domain objects. Installable with `pip install -e .`; use `sys.path.insert(0, 'src')` for in-tree development.
...
Renders an `Utterance` as a box-drawing syntax tree. Optional ANSI colour: tags bold, word forms blue, phon/script tags magenta, lemma IDs yellow. `print_tree` auto-detects TTY when `colour=None`.
...
for utt, cl in doc.find_by_form("para"):
    print(utt.sentence_id, cl)
```

See [`notebooks/coj_usage.ipynb`](notebooks/coj_usage.ipynb) for a fully executed walkthrough.

---

### Package-based processors

The scripts under `scripts/processors/` import `src/coj` and read/write `data/xml/` directly. They require `pip install -e .` first.
...
Utterance headers in `.txt` follow the pattern `=N("…")`; in XML the `<block header="…">` attribute stores only the bare word list.
...
Multi-valued fields (may repeat): `.FORM`, `.KANA`, `.MEANING`, `.COMPOUND`, `.RELATED`, `.DERIVATION`, `.TRANSREL`, `.NOTE`, `.VCLASS`, `.ITYPE`, `.POS`, `.GEO`, `.PTR`.

#### Lemma IDs

Format: `<PREFIX><zero-padded-6-digit-number>[optional-letter-suffix]` — e.g. `L000006a`, `N000001`.
...
src/coj/                          — Python package (pip install -e .)
  common/                         — shared utilities (ANSI colour helpers, …)
  core/                           — domain objects (corpus, dictionary, kana, …)
  xml/                            — XML serialisation helpers
  visual/                         — ascii_tree, print_tree
notebooks/
...
pyproject.toml                    — package config and ruff linter settings
```
