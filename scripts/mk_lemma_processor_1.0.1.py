...
NORMALISE_EXISTING = True   # True  → scan existing MK entries missing
                             #         .COMPOUND/.RELATED and fill them in
...
PHON_TAGS     = {"PHON", "LOG", "PHON-KUN", "PHON-KAN"}
...
def build_mk_entry(new_id: str,
    ...
    for lid, ph in related_info:
        body.append(f".RELATED\tref_target={lid}\t{ph}\n")
    body.append("\n")
...
def normalise_entry(entry: DictEntry,
    ...
    """
    If *entry* is a makura-kotoba entry missing .COMPOUND or .RELATED,
    ...
    if not has_related:
        ...
        entry.lines = [ln for ln in entry.lines if not ln.startswith(".RELATED")]
        ins_lines = [f".RELATED\tref_target={lid}\t{ph}\n"
                     for lid, ph in related_info]
        _insert_before_blank(entry, ins_lines)
        added.extend(f".RELATED\tref_target={lid}\t{ph}" for lid, ph in related_info)
...
def process_files() -> None:
    ...
    # ── NEW: map .FORM value → existing dictionary ID ─────────────────────────
    ...
    for fname in txt_files:
        ...
        while i < len(lines):
            ...
            if form_tmp in form_cache:
                ...
                # mk already exists in dictionary — reuse its ID
                new_id = form_to_existing_id[form_tmp]
                form_cache[form_tmp] = new_id   # cache to avoid repeated lookups
            else:
