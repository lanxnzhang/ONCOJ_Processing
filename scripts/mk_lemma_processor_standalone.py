...
RELATED_TOP_N = 1   # how many top-frequency target words to emit
...
REPORT_DICT_MODIFIED  = "report_dict_modified.txt"   # Report 2: entries normalised
REPORT_DICT_ADDED     = "report_dict_added.txt"      # Report 3: entries added
...
PHON_TAGS     = {"PHON", "LOG", "PHON-KUN", "PHON-ON",
...
def line_ends_with_word(fields: list[str]) -> bool:
    if not fields:
        return False
    return bool(WORD_FORM_RE.match(fields[-1]))
...
# ══════════════════════════════════════════════════════════════════════════════
#  MKTARGET WORD  (first word immediately following the MK block)
# ══════════════════════════════════════════════════════════════════════════════

def find_mktarget_words(block_end_idx: int,
                        all_lines: list[str],
                        top_n: int) -> list[tuple[str, str]]:
    """
    Walk forward from block_end_idx+1 to find the first word-bearing line(s).
    Returns up to top_n (lemma_id, phon) pairs.
    """
...
def find_mktarget_words_multi(block_end_indices: list[int],
                               all_lines: list[str],
                               top_n: int) -> list[tuple[str, str]]:
    ...
    for end_idx in block_end_indices:
        for lid, phon in find_mktarget_words(end_idx, all_lines, top_n=1):
            freq[phon] += 1
...
def normalise_entry(entry: DictEntry,
    ...
    has_mktarget   = entry.has_tag("MKTARGETNEW")
    ...
    if not has_mktarget:
        mktarget_info = find_mktarget_words_multi(block_end_indices, all_lines, top_n)
...
def process_files() -> None:
    ...
    # Map .FORM → existing dictionary ID (for existing MK entries)
    form_to_existing_id: dict[str, str] = {}
    for e in entries:
        form_val = e.get_field("FORM")
        if form_val:
            form_to_existing_id[form_val] = e.eid

    # Reserve all numeric parts found in text files
    ...
    # ── PASS 1: replace TARGET_ID ─────────────────────────────────────────────
    for fname in txt_files:
        total_files += 1
        lines = list(all_file_lines[fname])
        ...
        while i < len(lines):
            ...
            elif form_tmp in form_to_existing_id:
                new_id = form_to_existing_id[form_tmp]
            ...
            if CREATE_DICT_ENTRIES and new_id not in by_id:
                ...
                mktarget_info = find_mktarget_words(block_end, lines, RELATED_TOP_N)
    ...
    if OVERWRITE_SOURCE:
        ...
        dict_save = os.path.join(dict_out_dir, os.path.basename(DICT_FILE))
    ...
    # Report 2: dictionary entries modified (normalised)
    ...
    # Report 3: new dictionary entries added
...
if __name__ == "__main__":
    print("mk_lemma_processor_2.0.0")
    print("=" * 55)
